module.exports = function(grunt){
  ...
  grunt.registerTask('build', 'Build the add-on', function() {
    ...
    grunt.util.spawn({
      ...
      args: ['xpi',
        '--update-link',
        'https://people.mozilla.com/~bwinton/tile-switcher/tile-switcher.xpi',
        '--update-url',
        'https://people.mozilla.com/~bwinton/tile-switcher/tile-switcher.update.rdf'
      ]
  ...
  grunt.registerTask('copy', 'Copy the files to the server.', function() {
    ...
    if (!grunt.file.exists('/Volumes/People/public_html/tile-switcher')) {
      grunt.log.error('Missing Directory!');
      done(false);
      return;
    }

    grunt.file.copy('tile-switcher.xpi',
                    '/Volumes/People/public_html/tile-switcher/tile-switcher.xpi',
                    {"encoding": null});
    grunt.log.ok('Copied XPI…');
    grunt.file.copy('tile-switcher.update.rdf',
                    '/Volumes/People/public_html/tile-switcher/tile-switcher.update.rdf',
                    {"encoding": null});
    grunt.log.ok('Copied update.rdf…');
    grunt.file.copy('index.html',
                    '/Volumes/People/public_html/tile-switcher/index.html',
                    {"encoding": null});
